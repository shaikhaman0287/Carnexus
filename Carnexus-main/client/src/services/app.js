...
const localServer = isFileProtocol ? 'localhost:5001' : window.location.host;
const localApiBase = `${window.location.protocol}//${localServer}/api`;
...
window.loadDashboardStats = async function() {
    ...
    try {
        ...
        if (totalEl) totalEl.innerHTML = '<span style="color:#e74c3c; font-size: 0.9rem;">Server Error</span>';
        if (availEl) availEl.textContent = 'Err';
        if (soldEl)  soldEl.textContent  = 'Err';
        if (maintEl) maintEl.textContent = 'Err';
    }
};
